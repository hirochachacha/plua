...
local function checksyntax (prog, extra, token, line)
  ...
  -- local pt = string.format([[^%%[string ".*"%%]:%d: .- near %s$]],
						   -- line, token)
  local pt = string.format([[%%[string ".*"%%]:%d]], line)
...
-- TODO or spec: no way to support
...
do
  ...
  -- TODO or spec
  assert(not res and string.find(msg, "boolean expected"))
...
assert(c > 255 and string.find(b, "too many upvalues"))
